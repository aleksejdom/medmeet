...
agent_communication:
    ...
    - agent: "main"
      message: "Fixed dashboard redirection after leaving video call. The leaveVideoCall function now explicitly sets view to 'dashboard', clears the active call, and reloads user-specific data. This ensures users are properly returned to their dashboard after ending a video consultation. PeerJS implementation is complete with no Jitsi branding."
    - agent: "testing"
      message: "REGRESSION TEST COMPLETED: All critical backend endpoints working perfectly after PeerJS implementation. Tested authentication (register, login, session check, logout), time slot management (create, get), appointment management (book, get), and additional endpoints (doctors list). 100% success rate on critical functionality. No regressions detected. Backend APIs remain fully functional."